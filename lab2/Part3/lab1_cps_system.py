...
def log():
    ...
    msg = f"{{{time_second}, room1, lights, {room1_state}}}"
    client.publish(TOPIC_LIGHTS, msg.encode())
    client.publish(TOPIC, msg.encode())
    
    room2_state = "on" if (green_value == 1) else "off"
    msg = f"{{{time_second}, room2, lights, {room2_state}}}"
    client.publish(TOPIC_LIGHTS, msg.encode())
    client.publish(TOPIC, msg.encode())
    
    room3_state = "on" if (white_value == 1) else "off"
    msg = f"{{{time_second}, livingroom, lights, {room3_state}}}"
    client.publish(TOPIC_LIGHTS, msg.encode())
    client.publish(TOPIC, msg.encode())
    
    room4_state = "on" if (red_value == 1) else "off"
    msg = f"{{{time_second}, garage, lights, {room4_state}}}"
    client.publish(TOPIC_LIGHTS, msg.encode())
    client.publish(TOPIC, msg.encode())
    
    msg = f"{{{time_second}, thermostat, temperature, {temperature}}}"
    client.publish(TOPIC_TEMP, msg.encode())
    client.publish(TOPIC, msg.encode())
    
    alarm_state = "on" if alarm_on else "off"
    msg = f"{{{time_second}, garage, alarm, {alarm_state}}}"
    client.publish(TOPIC_ALARM, msg.encode())
    client.publish(TOPIC, msg.encode())
